...
type Case = { id:string; ticket:string; todo:boolean; notes:string; prepared?:any; analysis?:any; translated?:any; language:"ZH"|"ORIGINAL"; loading?:string; error?:string; diagnostic?:string; generations:Record<string,number> };
...
export function App() {
  ...
  return <main>
    ...
    <section className="status">{current.loading || current.error || "Ready"}{current.error?.includes("authentication")&&<button onClick={()=>setTokenOpen(true)}>Update Nextop Token</button>}{current.diagnostic&&<details className="diagnostic" open><summary>Help diagnose this failure</summary><pre>{current.diagnostic}</pre><button onClick={()=>navigator.clipboard.writeText(current.diagnostic || "")}>Copy diagnostic</button></details>}</section>
