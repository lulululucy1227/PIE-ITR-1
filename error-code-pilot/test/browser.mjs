...
try {
 ...
 await run('public payload excludes candidate/internal metadata and raw endpoint',async()=>{
  const {context,page}=await fresh();await page.locator('#search-button:enabled').waitFor();const payload=await (await page.request.get(base+'/knowledge.json')).text();assert.doesNotMatch(payload,/label_cn|evidence_state|sourceRefs|raw_reference|STABLE_OPERATIONAL_GUIDANCE|CONFLICTING_EVIDENCE|candidate_refs|REP-[A-Z0-9]+-\d{3}|feishu-candidates|repeated_use_signal/);assert.equal(publicData.symptoms.length,25);for(const url of ['/data/canonical.json','/data/feishu-candidates.json','/lib/candidates.mjs'])assert.equal((await page.request.get(base+url)).status(),404);await context.close();
 });
