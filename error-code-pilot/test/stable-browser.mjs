...
async function selectSymptom(page,card){const s=data.symptoms.find(s=>s.repair_refs.some(r=>r.card_id===card.id));await page.locator(`[data-area="${s.observable_area}"]`).click();await page.locator(`[data-symptom="${s.symptom_id}"]`).click();if(s.repair_refs.length>1)await page.getByRole('button',{name:card.paths[0].symptom,exact:true}).click();}
...
try{
 for(const width of [1366,1920]){
  ...
  for(const card of cards){
   ...
   await page.locator('#model').selectOption('__other__');await continueToSolution(page,card);assert.equal(await page.locator('#result .answer-grid').count(),0,'unknown model');
   await page.locator('#edit-issue').click();await page.locator('#model').selectOption(card.scope.models[0]);await confirm(page,card);await continueToSolution(page,card);
