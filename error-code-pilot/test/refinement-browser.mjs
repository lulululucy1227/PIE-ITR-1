...
try {
 for(const width of [1366,1920,390]) {
  ...
  assert.deepEqual(await symptoms.locator('optgroup[label="Cutting disc & height"] option').evaluateAll(xs=>xs.map(x=>x.value)),['SYM-004','SYM-005','SYM-006']);
  assert.equal(await symptoms.locator('optgroup[label="Wheels & movement"]').count(),1);
  assert.equal(await symptoms.locator('option[value="__other__"]').count(),1);
