...
const fields=()=>({model:$('model').value,area:catalog?.symptoms.find(s=>s.symptom_id===$('observed-symptom').value)?.observable_area||'',symptomId:$('observed-symptom').value});
...
function updateSelectionSummary(){const values=[fields().model,areaLabel[fields().area],symptomLabel(catalog?.symptoms.find(s=>s.symptom_id===fields().symptomId))].filter(Boolean);$('selection-summary').textContent=values.length?values.join(' · '):'Choose your model, then what you notice.';}
function setStage(next){stage=next;for(const id of ['category','symptom','confirm'])$(id+'-stage').hidden=id!==next;
 const categoryReady=!!catalog&&controlledModels(catalog).includes(fields().model);
...
function refreshSymptoms(){const select=$('observed-symptom');select.replaceChildren(new Option('Select a symptom',''));for(const area of OBSERVABLE_AREAS){const symptoms=controlledSymptoms(catalog,{model:fields().model,area});if(!symptoms.length)continue;const group=el('optgroup');group.label=areaLabel[area];for(const s of symptoms)group.append(new Option(symptomLabel(s),s.symptom_id));select.append(group);}select.append(new Option('Other / None of these','__other__'));$('other-field').hidden=true;$('other-description').value='';}
function categoryChanged(modelChanged){invalidate();clearSelection();mower.model=controlledModels(catalog).includes(fields().model)?fields().model:'';if(modelChanged){$('firmware').value='';mower.firmware='';}$('more-details').hidden=true;resetQualifiers();refreshSymptoms();setStage(mower.model?'symptom':'category');history.replaceState(marker('identify'),'','#identify');}
function nextCategory(){if(!catalog||!controlledModels(catalog).includes(fields().model)){status('Choose a model from the list.',true);return;}setStage('symptom');history.replaceState(marker('identify'),'','#identify');focusResult();}
function detailsEdited(){invalidate();clearSelection();resetQualifiers();$('more-details').hidden=true;$('firmware').value='';mower.firmware='';$('other-field').hidden=fields().symptomId!=='__other__';if(stage==='confirm')setStage('symptom');else updateSelectionSummary();history.replaceState(marker('identify'),'','#identify');}
...
function beginIdentification(){if(resolved())renderIdentification();else prepareIdentification();}
...
function renderIdentification(){const result=resolved();if(!result){clearSelection();setStage('symptom');return;}if(!['scope_required','choose_symptom','qualifier_required','choose_path'].includes(result.kind)){$('selection').replaceChildren();setStage('symptom');viewSolution();return;}setStage('confirm');const box=el('article',undefined,'card');const state=selected&&visit(selected);
 ...
 else if(result.kind==='qualifier_required'){
 // Condense only the reviewed exact conditions; future qualifier changes retain their full prompt.
 const hardware=selected.id==='guide-cutting-operation'&&result.prompt==='No Error 1202 is present, and the cutting problem occurs during actual mowing or manual operation, not only in Functional Test.',software=selected.id==='sym-cutting-functional-test'&&result.prompt==='The cutting function works normally in Auto Run and/or manual mowing, fails only in Functional Test, and no other motor or driver fault is present.';
 const question=hardware?'When does the cutting problem occur?':software?'Does the problem occur outside Functional Test?':'Which situation applies?';
 box.append(el('h3',question));if(!hardware&&!software)box.append(el('p',result.prompt));
 const answers=hardware?['During mowing / manual operation, with no Error 1202','Only in a test, Error 1202 is shown, or unsure']:software?['Mowing works normally; no other motor or driver fault','Mowing also fails, another fault is present, or unsure']:result.choices.map(c=>c.label);
 for(const [index,c] of result.choices.entries()){const choice=button(answers[index],()=>{if(!valid()||resolutionKey!==inputsKey())return;Object.assign(state,{qualifierConfirmed:c.id==='yes',qualifierRejected:c.id!=='yes'});renderIdentification();});choice.id='qualifier-'+c.id;box.append(choice);}
 }
 ...
 const needsFirmware=!!selected?.scope.firmware.length;$('scope-details').hidden=!needsFirmware;$('more-details').hidden=!needsFirmware;if(needsFirmware){$('scope-details').append($('more-details'));$('more-details').open=true;}
...
function renderCard(override){if(!valid()||resolutionKey!==inputsKey()){invalidate();clearSelection();setStage('category');return;}clearResult();const box=el('article',undefined,'card');box.dataset.card=selected.id;const top=el('div',undefined,'card-top');top.append(el('span',selected.code?'ERROR '+selected.code:'SYMPTOM GUIDE','code-label'),button('Edit issue',editIssue,'text-button'));box.append(top,el('h2',selected.message));
 ...
 const outcome=el('div',undefined,'outcome');const buttons=el('div',undefined,'buttons');const outcomeKey=inputsKey(),outcomeCard=selected,outcomePath=current.pathId;const act=choice=>{if(!valid()||outcomeKey!==inputsKey()||selected!==outcomeCard||current?.pathId!==outcomePath){invalidate();clearSelection();setStage('symptom');return;}const pathId=current.pathId;const next=recordOutcome(selected,pathId,choice,{...context(),verificationComplete:choice==='fixed'});if(next.kind==='verification_required'){status(next.message,true);buttons.querySelector('button')?.focus();return;}if(choice==='not_fixed')state.completedRepairs=[...new Set([...state.completedRepairs,pathId])];if(choice==='returned')state.returned=true;if(next.pathId)state.pathId=next.pathId;else if(!state.pathId)state.pathId=pathId;if(choice!=='fixed')state.qualifierConfirmed=false;renderCard(next);status(choice==='fixed'?'Result shown for this visit only. No case record was changed.':'Use the next step shown below.');focusResult();};buttons.append(button('Checks passed — issue resolved',()=>act('fixed'),'primary'),button('Still not fixed',()=>act('not_fixed'),'secondary'));if(current.kind!=='information')buttons.append(button('The issue returned after repair',()=>act('returned'),'text-button'));outcome.append(buttons);box.append(outcome);
...
$('model').addEventListener('change',()=>categoryChanged(true));
...
try{const response=await fetch('./knowledge.json',{cache:'no-store'});if(!response.ok)throw new Error('Guide load failed');const data=await response.json();if(!validCatalog(data))throw new Error('Invalid guide data');catalog=data;
 for(const model of controlledModels(catalog))$('model').append(new Option(model,model));
 for(const id of ['model','observed-symptom','firmware','search-button','search'])$(id).disabled=false;status('');
