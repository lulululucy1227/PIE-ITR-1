...
if(!Number.isInteger(port)||port<1024||port>65535||port===8787)throw new Error('Choose an unoccupied specialist port; never8787.');
const results=[];
for(let cycle=1;cycle<=2;cycle++){
 const child=spawn(process.execPath,['scripts/serve.mjs',String(port)],{cwd:root,stdio:['ignore','pipe','pipe'],windowsHide:true});
 let stdout='',stderr='';child.stdout.on('data',b=>stdout+=b);child.stderr.on('data',b=>stderr+=b);
 try{
  await new Promise((resolve,reject)=>{
   const timeout=setTimeout(()=>reject(new Error('Pilot startup timed out')),5000);
   child.stdout.on('data',()=>{if(stdout.includes('http://127.0.0.1:'+port)){clearTimeout(timeout);resolve();}});
  ...
  results.push({cycle,port,httpStatus:res.status});
 }finally{
...
console.log('PASS two start -> owned exit -> relaunch cycles on '+port);
