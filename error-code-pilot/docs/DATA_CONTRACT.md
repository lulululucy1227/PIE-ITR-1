...
Each candidate's independent `review` records evidence_state, visibility, rationale, source_refs, last_reviewed, promoted_refs, known_conflicts_or_reopens, scope/verification/fallback confirmation, active_maintenance and nullable operational history. Task004 reviewed the full original path. Under the later task006 supervisor decision, `publication_scope` identifies exactly which supported portion and symptom/model branches are approved; flags and reciprocal promoted_refs apply to those scoped portions, not the complete original fallback chain. `previous_review` preserves the earlier decision and rationale. The supervisor-accepted operational-maturity signal is explicitly attributed; numerical use counts and earliest-use dates remain unknown. Empty conflicts describes the reviewed scoped portion, not an exhaustive real-case reopen audit.
...
Task006 publishes9 candidate portions through10 canonical cards (7 repair,3 check); the CHARGE parent has two symptom/model-scoped references. Check paths keep part=null and show their procedure/verification without a manufactured faulty-part box. New family selectors reflect source-declared families only; they do not assert part or capacity compatibility. Each action still requires model-supported tooling and a confirmed serviceable faulty element before replacement. POS/WIFI/LIDAR and WATER/PHY remain PIE-only despite accepted process maturity; five weaker candidates remain SOURCE_RECOMMENDATION. REP-1202-001 remains CONFLICTING_EVIDENCE/WITHHELD.
