...
### Canonical facts should state the correct value directly
对于密码、料号、版本号、兼容关系、正式名称、固定参数、供应规则等“单一正确值”知识，Knowledge 应直接陈述正确值，不要为了复述原工单过程而同时保留已知错误值。

例如：如果正式密码是 `MMTT`，则标题、快速判断、已知结论和处理方法只应告诉客服使用 `MMTT`。除非“错误值本身会造成可复现故障并且识别该错误值对诊断有直接价值”，否则不要写成“不是 MMIT，而是 MMTT”。

原则：
- 客服知识库优先提供 canonical fact，而不是复述用户曾经输错什么；
- 已知错误值、误拼写、误操作属于 Case History 时，默认留在 Evidence，不进入面向客服的核心 Knowledge；
- 避免在标题和快速判断中重复传播错误密码、错误料号、错误版本或错误命名；
- 对“正确值是什么”这类知识，最短且准确的表达优先。

### Fault classification must be semantically faithful
`故障类别 / 故障模块` 是客服检索入口，不是为了让 Topic ID 非空而强行寻找“最接近”的标签。

分类时必须优先匹配：
1. 客服可观察到的主要问题；
2. Knowledge 实际解决的对象或功能；
3. 当前标签体系中语义真正一致的类别/模块。

禁止：
- 因为部件名称相近就错分到无关模块；
- 因为最终更换了某部件就忽略用户实际遇到的故障入口；
- 为了让 Topic ID 生成而把前灯、LED、底盘、进水等问题硬塞进视觉/雷达、轮毂等不相干模块；
- 将“当前没有合适标签”误当成执行失败。

如果现有标签体系没有合理位置，应保持 `TAXONOMY GAP`，而不是错误分类。`Topic ID` 为空可以接受，错误 Topic 不可以接受。
