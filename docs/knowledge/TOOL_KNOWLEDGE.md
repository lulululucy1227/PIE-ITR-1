...
- MammoSuite **cannot test the left and right cutting-disc motors separately**. Do not instruct an agent to isolate left/right cutting motors independently in MammoSuite Functional Test.
...
- When a Mammotion Kit / Function Test report shows a cutting-motor value of `0` but the cutting system works normally in **Auto Map Run** and can stop/restart correctly after an obstacle event, treat the evidence as contradictory. Do **not** diagnose the cutting motor as confirmed failed from the `0` value alone.
- In that contradictory state, **Burn-in Test** is a valid next diagnostic step in Mammotion Kit. Current PIE workflow: connect the mower by cable and start Burn-in Test; disconnect the cable and let the mower run for about 20 minutes; reconnect the cable; read the Burn-in Test results; then upload a fresh log after the test.
- Use the Burn-in result together with the real-operation behavior and fresh log to decide whether the cutting path is actually faulty. Until the Burn-in result is available, this is a diagnostic escalation method, not a confirmed repair solution.
...
- Firmware update failure can sometimes be caused by an unstable network connection or slow download/response from the server. Do not treat an update failure by itself as proof of a mower hardware fault.
- For firmware updates, especially after a mainboard replacement or when wireless updating is inconsistent, prefer **Mammotion Kit Wired**. The wired connection communicates directly with the mower and is generally more stable than Bluetooth for firmware updating and module synchronization.
